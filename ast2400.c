...
static int ast2400_spi_send_command(const struct flashctx *flash,
...
static int ast2400_init_v1(const struct programmer_cfg *cfg)
{
	...
	dev = pcidev_init(cfg, bmc_aspeed_ast2400, PCI_BASE_ADDRESS_1);
	if (!dev)
		return 1;
	...
	register_spi_master(&spi_master_ast2400, NULL);

	return 0;
}

static void ast2400_spi_xfer_data(const struct flashctx *flash,
				   unsigned int writecnt, unsigned int readcnt,
				   const unsigned char *writearr,
				   unsigned char *readarr)
{
...
const struct programmer_entry programmer_ast2400 = {
	.name			= "ast2400",
	.type			= PCI,
	.devs.dev		= bmc_aspeed_ast2400,
	.init			= ast2400_init_v1,
};
