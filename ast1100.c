...
static const struct dev_entry bmc_aspeed_ast1100_alt[] = {
	{PCI_VENDOR_ID_ASPEED, 0x2000, OK, "ASPEED", "AST1100"},

	{0},
};

static int ast1100_spi_send_command(const struct flashctx *flash,
...
static int ast1100_init_1(const struct programmer_cfg *cfg){
	...
	dev = pcidev_init(cfg, bmc_aspeed_ast1100_alt, PCI_BASE_ADDRESS_1);
	if (!dev)
		return 1;
	...
	register_spi_master(&spi_master_ast1100, NULL);
				   ...
				   unsigned char *readarr)
{
	...
	ast1100_spi_xfer_data((struct flashrom_flashctx *)flash, writecnt, readcnt, writearr, readarr);
...
const struct programmer_entry programmer_ast1100= {
	.name			= "ast1100",
	.type			= PCI,
	.devs.dev		= bmc_aspeed_ast1100_alt,
	.init			= ast1100_init_1,
};
